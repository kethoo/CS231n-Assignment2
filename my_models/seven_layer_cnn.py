#7 layerians ვქმნი, რადგან წესით აქ უნდა
# მოხდეს overfit
import torch
...
class SevenLayerCNN(nn.Module):
    def __init__(self, num_classes=emotion_const):
        ...
        self.fc2 = nn.Linear(fc_const_mine, 256)  
    ...
    def forward(self, x):
       ...
       #2
        ...
        #6
        kv6 = self.conv6(x)
        kv6_relu = plw.relu(kv6)
        x = kv6_relu
        #7
        kv7 = self.conv7(x)
        kv7_relu = plw.relu(kv7)
        x = kv7_relu
        inp_br_x = x.size(0)
        x = x.view(inp_br_x, -1)
    ...
    def curr_m_params(self):  
        return { 
            'model_name': 'SevenLayerCNN',  
            'details': '7 layered CNN - Deep architecture ',  
            'my_prediction': 'Will definitely overfit without regularization',
            'all_params': self.parameters_to_train(),
            'layers used': ['Conv2d(1->16, 3x3)', 'MaxPool2d(2x2)','Conv2d(16->32, 3x3)', 'MaxPool2d(2x2)', 'Conv2d(32->64, 3x3)','Conv2d(64->128, 3x3)', 'MaxPool2d(2x2)','Conv2d(128->256, 3x3)','Conv2d(256->512, 3x3)','Conv2d(512->1024, 3x3)','Linear(36864->1024)', 'Linear(1024->256)', 'Linear(256->7)'
            ],
        }
...
def build_SevenLayerCNN(num_classes=emotion_const):
    return SevenLayerCNN(num_classes)  

def chemi_preds_final():
    print("7 ლეიერიანი CNN-ისთვის ")
    print("ეს არის clean version dropout-ის გარეშე")
    print("წესით ახლა ძალიან ბევრი overfitting უნდა მოხდეს")
    print("შესანიშნავი მაგალითი იქნება regularization-ის საჭიროებისა")

if __name__ == "__main__":
    print("Seven layer CNN is loading...")
    curr_m = build_SevenLayerCNN()

...
#channel attention mechanism ჩავუმატე
import torch
...
class FiveLayerAttention(nn.Module):
    def __init__(self, num_classes=emotion_const):
        super(FiveLayerAttention, self).__init__()  
    ...
    def forward(self, x):
        #1
        kv1 = self.conv1(x)
        kv1_att = self.att1(kv1)      
        kv1_relu = plw.relu(kv1_att)  
        x = self.pool1(kv1_relu)
        #2
        kv2 = self.conv2(x)
        kv2_att = self.att2(kv2)      
        kv2_relu = plw.relu(kv2_att)
        x = self.pool2(kv2_relu)
        #3
        kv3 = self.conv3(x)
        kv3_att = self.att3(kv3)      
        kv3_relu = plw.relu(kv3_att)
        x = kv3_relu
        # 4
        kv4 = self.conv4(x)
        kv4_att = self.att4(kv4)    
        kv4_relu = plw.relu(kv4_att)
        x = self.pool4(kv4_relu)
        #5
        kv5 = self.conv5(x)
        kv5_att = self.att5(kv5)     
        kv5_relu = plw.relu(kv5_att)
        x = kv5_relu
        inp_br_x = x.size(0)
        x = x.view(inp_br_x, -1)
        x = self.dropout(x)
    ...
    def curr_m_params(self):  
        return { 
            'model_name': 'FiveLayerWithChannelAttention',  
            'details': '5 layer cnn with additional channel attention',  
            'my_prediction': 'Probably works better than 5 layer with batch normaliztion',
            'all_params': self.parameters_to_train(),
            'layers used': [
                'Conv2d(1->32, 3x3)', 'ChannelAttention(32)', 'MaxPool2d(2x2)','Conv2d(32->64, 3x3)', 'ChannelAttention(64)', 'MaxPool2d(2x2)', 'Conv2d(64->128, 3x3)', 'ChannelAttention(128)',
                'Conv2d(128->256, 3x3)', 'ChannelAttention(256)', 'MaxPool2d(2x2)','Conv2d(256->512, 3x3)', 'ChannelAttention(512)','Linear(18432->512)', 'Linear(512->128)', 'Linear(128->7)'
            ],
        }
...
def build_FiveLayerAttention(num_classes=emotion_const):
    return FiveLayerAttention(num_classes)  

def chemi_preds_final():
    print("ამჟამად 5 ლეიერიანს დავუმატეთ attention კომპონენტი")
    print("რადგანაც ეთენშენის მთავარი დანიშნულებაა გადაარჩიოს და შეისწავლოს რომელი features არის მნიშვნელოვანი,")
    print("ეს მოდელი საკმაოდ კარგად იმუშავებს")
    

if __name__ == "__main__":
    print("Five layer with additional Channel Attention CNN is loading...")
    curr_m = build_FiveLayerAttention()

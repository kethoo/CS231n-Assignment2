# 5-layer CNN + BatchNorm + Dropout
# ეს არის ყველაზე ოპტიმიზებული ვერსია - batch normalization + dropout ერთად
import torch
...
class FiveLayerBatchNormDropout(nn.Module):
    def __init__(self, num_classes=emotion_const):
        super(FiveLayerBatchNormDropout, self).__init__()  
    ...
    def curr_m_params(self):  
        return { 
            'model_name': 'FiveLayerBatchNormDropout',  
            'details': '5 layer CNN with batch normalization and dropout - Best regularization combo',  
            'my_prediction': 'Most balanced model - stable training with minimal overfitting',
            'all_params': self.parameters_to_train(),
            'optimizations': [
                'BatchNorm after each conv layer', 
                'Spatial dropout (25%) after conv5',
                'FC dropout: 50% after FC1, 30% after FC2'
            ],
            'layers used': [
                ...
                'Conv2d(256->512, 3x3)', 'BatchNorm2d(512)', 'Dropout2d(25%)',
...
def build_FiveLayerBatchNormDropout(num_classes=emotion_const):
    return FiveLayerBatchNormDropout(num_classes)  

def chemi_preds_final():
    print("ეს არის ყველაზე ოპტიმიზებული მოდელი - BatchNorm + Dropout კომბინაცია")
    print("BatchNorm -> სტაბილური training")
    print("Dropout -> ხელს უშლის overfitting-ს")
    print("ერთად ისინი ქმნიან კარგ მოდლეს, რომელსად კარგი პერფორმანსი აქვს და გენერალიზაციაც შეუძლია")

if __name__ == "__main__":
    print("Five layer CNN with BatchNorm and Dropout is loading...")
    curr_m = build_FiveLayerBatchNormDropout()
